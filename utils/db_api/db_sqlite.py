...
class DataBase:
    ...
    def tovar_count(self, tovar_id):
        with self.connection:
            result = self.cursor.execute(f'SELECT "tovar_count" FROM "tovar" WHERE "tovar_id" = "{tovar_id}" ').fetchall()[0]
            return result[0] if len(result) > 0 else []
...
class User:
    ...
    def add_tovar_(self, tovar_id, user_id, tovar_name, tovar_price=0, count=0):
        with self.connection:
            ...
            if len(result) != 0:
                ...
                self.cursor.execute(f'INSERT INTO "{user_id}" '
                                    f'VALUES ("{tovar_id}", "{tovar_name}", "{count}", "{tovar_price}", 0)')
